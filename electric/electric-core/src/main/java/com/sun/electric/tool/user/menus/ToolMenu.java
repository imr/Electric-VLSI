...
public class ToolMenu {
	...
	static EMenu makeMenu() {
		...
		expRoutingMenu = new EMenu("Experimental Routers",
				...
				new EMenuItem("Experimental Sea Of Gates with PI") {
					public void run() {
						SeaOfGates.seaOfGatesRoute(SeaOfGatesEngineType.newInfrastructure3);
					}
		...
		// {
		// JMenu menu =
		// (JMenu)menuBarInstance.findMenuItem(expRoutingMenu.getPath());
		// RoutingFrame[] expRouters = RoutingFrame.getRoutingAlgorithms();
		// for(int i=0; i<expRouters.length; i++)
		// {
		// EMenuItem elem = new
		// DynamicExperimentalRoutingMenuItem(expRouters[i]);
		...
		// ------------------- DRC
				...
				new EMenu("_DRC", new EMenuItem("Check _Hierarchically", KeyStroke.getKeyStroke(
						KeyEvent.VK_F5, 0)) {
					public void run() {
						...
						if (dp.isMultiThreaded && cell.isLayout()) {
							...
							DRC.checkDRCHierarchically(dp, cell, null, null,
					...
					public void run() {
						LayerCoverageTool.layerCoverageCommand(WindowFrame.needCurCell(),
								GeometryHandler.GHMode.ALGO_SWEEP, true,
				...
				SEPARATOR, new EMenuItem("Import _Assura DRC Errors for Current Cell...") {
					public void run() {
				...
				SEPARATOR, new EMenuItem("Export DRC Deck...") {
					public void run() {
						exportDRCDeck();
					}
				...
				new EMenu("Simulation (Built-in)", IRSIM.hasIRSIM() ? new EMenuItem(
						"IRSI_M: Simulate Current Cell") {
					public void run() {
						SimulationTool.startSimulation(SimulationTool.IRSIM_ENGINE, null, null, null, false);
					}
					...
					public void run() {
						SimulationTool.startSimulation(SimulationTool.IRSIM_ENGINE, "", null, null, false);
					}
				} : null,
				...
				}, SEPARATOR, new EMenuItem("Set Verilog _Template") {
					public void run() {
				...
				}, SEPARATOR, !IRSIM.hasIRSIM() ? new EMenuItem("Write _IRSIM Deck...") {
					public void run() {
						FileMenu.exportCommand(FileType.IRSIM, true);
					}
				...
				}, SEPARATOR, new EMenuItem("Write _FastHenry Deck...") {
					public void run() {
				...
				}, SEPARATOR, new EMenuItem("Copy Schematic _User Names to Layout") {
					public void run() {
				...
				// mnemonic keys available: A CDE H KLM O Q U XYZ
						new EMenu("Add NCC _Annotation to Cell", new EMenuItem(
								"Exports Connected by Parent _vdd") {
							public void run() {
								NccCellAnnotations
										.makeNCCAnnotationMenuCommand("exportsConnectedByParent vdd /vdd_[0-9]+/");
							}
						}, new EMenuItem("Exports Connected By Parent _gnd") {
							public void run() {
								NccCellAnnotations
										.makeNCCAnnotationMenuCommand("exportsConnectedByParent gnd /gnd_[0-9]+/");
							}
							...
							public void run() {
								NccCellAnnotations
										.makeNCCAnnotationMenuCommand("skipNCC <comment explaining why>");
							}
						}, new EMenuItem("_Not a Subcircuit") {
							public void run() {
								NccCellAnnotations
										.makeNCCAnnotationMenuCommand("notSubcircuit <comment explaining why>");
							}
						}, new EMenuItem("_Flatten Instances") {
							public void run() {
								NccCellAnnotations
										.makeNCCAnnotationMenuCommand("flattenInstances <list of instance names>");
							}
							...
							public void run() {
								NccCellAnnotations
										.makeNCCAnnotationMenuCommand("forcePartMatch <Part name shared by schematic and layout>");
							}
						}, new EMenuItem("Force _Wire Match") {
							public void run() {
								NccCellAnnotations
										.makeNCCAnnotationMenuCommand("forceWireMatch <Wire name shared by schematic and layout>");
							}
						}, new EMenuItem("_Black Box") {
							public void run() {
								NccCellAnnotations
										.makeNCCAnnotationMenuCommand("blackBox <comment explaining why>");
							}
						})),
				 ...
				 * , new EMenuItem("Redo Network N_umbering") { public void
				 * run() { NetworkTool.renumberNetlists(); }}
				 */),
				...
				}, SEPARATOR, new EMenuItem("_Load Logical Effort Libraries (Purple, Red, and Orange)") {
					public void run() {
				...
				new EMenuItem("Sea-Of-_Gates Route") {
					public void run() {
				...
				Routing.hasSunRouter() ? SEPARATOR : null, Routing.hasSunRouter() ? new EMenuItem(
						"Sun _Lava Router") {
					public void run() {
				...
				new EMenu("_Generation", new EMenuItem("_Coverage Implants Generator") {
					public void run() {
						layerCoverageCommand(LayerCoverageTool.LCMode.IMPLANT,
								GeometryHandler.GHMode.ALGO_SWEEP);
					}
				...
				}, SEPARATOR, new EMenuItem("Stitch-Based Fill Generator from doc input") {
					public void run() {
				...
				} : null, SEPARATOR, new EMenuItem("Generate gate layouts (_MoCMOS)") {
					public void run() {
						GateLayoutGenerator.generateFromSchematicsJob(Technology.getMocmosTechnology());
					}
				}, Technology.getTSMC180Technology() != null ? new EMenuItem(
						"Generate gate layouts (T_SMC180)") {
					public void run() {
						GateLayoutGenerator.generateFromSchematicsJob(Technology.getTSMC180Technology());
					}
				} : null, Technology.getCMOS90Technology() != null ? new EMenuItem(
						"Generate gate layouts (CM_OS90)") {
					public void run() {
					...
					public void run() {
						GateLayoutGenerator.generateFromSchematicsJob(Schematics
	...
	public static void listNetworksCommand() {
		...
		if (netlist == null) {
			System.out
					.println("Sorry, a deadlock aborted netlist display (network information unavailable).  Please try again");
			return;
		}
	...
	/**
	 * Method to create a new template on the target cell. Templates can be for
	 * SPICE or Verilog, depending on the Variable name.
	 * 
	 * @param templateKey
	 *            the name of the variable to create.
	...
	/**
	 * Method to create a new template on the target cell, with a predefined
	 * String or String []. Templates can be for SPICE or Verilog, depending on
	 * the Variable name.
	 * 
	 * @param templateKey
	 *            the name of the variable to create.
	...
	private static class DoSilCompActivity extends Job {
		...
		public static Cell performTaskNoJob(Cell cell, List<Cell> textCellsToRedraw, int activities,
				SilComp.SilCompPrefs prefs, GenerateVHDL.VHDLPreferences vp,
				boolean isIncludeVersionAndDateInOutput) throws JobException {
