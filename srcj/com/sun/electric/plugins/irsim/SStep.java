...
public class SStep extends Eval
{
	...
	public void modelEvaluate(Sim.Node n)
	{
		...
		for(Sim.Node thisOne = n; thisOne != null; thisOne = thisOne.nLink)
		{
			...
			if ((thisOne.nFlags & Sim.INPUT) == 0)
			{
				...
				if ((thisOne.nFlags & Sim.WATCHED) != 0 && (theAnalyzer.localPrefs.irDebug & (Sim.DEBUG_DC | Sim.DEBUG_EV)) != 0)
				{
	...
	private int scThev(Sim.Node n, int level)
	{
		...
		if ((theAnalyzer.localPrefs.irDebug & (Sim.DEBUG_DC | Sim.DEBUG_TW)) != 0 && level > 0)
		{
			System.out.print("  ");
			for(int i = level; --i > 0; )
				System.out.print(" ");
