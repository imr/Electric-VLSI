...
//        long[] result;
        if (techPool.getArcProto(a.protoId).isCurvable()) {
            ...
            if (radiusDouble != null && curvedArcGridOutline(a, gridWidth, DBMath.lambdaToGrid(radiusDouble))) {
                pushPoly(style, layer, graphicsOverride);
                return;
            }
        }
    ...
    public void pushPoly(Poly.Type style, Layer layer, EGraphics graphicsOverride) {
        addDoublePoly(pointCount, style, layer, graphicsOverride);
        pointCount = 0;
    }
