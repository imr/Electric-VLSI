...
public class ERCWellCheck2 {
	...
	public static int checkERCWell(Cell cell, GeometryHandler.GHMode newAlgorithm,
		...
		ERCWellCheck2 check = new ERCWellCheck2(cell, null, newAlgorithm, wellPrefs);
		return check.runNow();
	}

	public static int checkERCWell(Cell cell, WellCheckPreferences wellPrefs) {
		ERCWellCheck2 check = new ERCWellCheck2(cell, null, wellPrefs);
		return check.runNow();
	}

	@Deprecated
	private ERCWellCheck2(Cell cell, WellCheckJob job, GeometryHandler.GHMode newAlgorithm,
			WellCheckPreferences wellPrefs) {
		this.job = job;
		this.cell = cell;
	...
	private ERCWellCheck2(Cell cell, WellCheckJob job, WellCheckPreferences wellPrefs) {
		this.job = job;
		this.cell = cell;
		this.wellPrefs = wellPrefs;
		this.transistors = new HashMap<Integer, List<Transistor>>();
	...
	private static class WellCheckJob extends Job {
		...
		public boolean doIt() throws JobException {
			ERCWellCheck2 check = new ERCWellCheck2(cell, this, wellPrefs);
			check.runNow();
