 ...
 *
 * File: JemStratWiresWithWithoutGates.java
 *
...
/* 
 * JemStratWiresWithWithoutGates divides wires into those with gates and those
 ...
 */
public class JemStratWiresWithWithoutGates extends JemStrat {
    ...
    private JemStratWiresWithWithoutGates(NccGlobals globals) {super(globals);}

    // ---------- to do the job -------------

	public static void doYourJob(NccGlobals globals){
		JemStratWiresWithWithoutGates jsws = 
			new JemStratWiresWithWithoutGates(globals);
		jsws.doYourJob2();
	}
    ...
    private void preamble(JemEquivRecord j){
        startTime("JemStratWiresWithWithoutGates" , j.nameString());
    }

	//summarize at the end
    private void summary(JemLeafList offspring){
        globals.println(" JemStratWiresWithWithoutGates found " +
                       numNoGates + " Wires without gates and " +
                       numWithGates + " Wires with gates");
        globals.println(offspring.sizeInfoString());
		globals.println(" JemStratWiresWithWithoutGates offspring: ");
    ...
    public Integer doFor(NetObject n){
    	error(!(n instanceof Wire), "JemStratWiresWithWithoutGates expects only Wires");
		Wire w= (Wire)n;
