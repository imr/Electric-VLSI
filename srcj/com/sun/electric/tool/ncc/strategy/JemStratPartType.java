...
/* JemStratPartType partitions Part equivalence classes
 * based upon the Part's type. */
public class JemStratPartType extends JemStrat {
	private Map typeCodeToTypeName = new HashMap();
	
    private JemStratPartType(NccGlobals globals) {super(globals);}
    ...
    private void summary(JemLeafList offspring) {
        globals.println("JemStratPartType produced " + offspring.size() +
                        " offspring");
        if (offspring.size()!=0) {
    ...
    public Integer doFor(NetObject n){
    	error(!(n instanceof Part), "JemStratPartType expects only Parts");
		Part p = (Part) n;
		typeCodeToTypeName.put(new Integer(p.typeCode()), p.typeString());
		return new Integer(p.typeCode());
    }

	// ------------------------- intended inteface ----------------------------
	public static JemLeafList doYourJob(NccGlobals globals){
		JemStratPartType pow = new JemStratPartType(globals);
