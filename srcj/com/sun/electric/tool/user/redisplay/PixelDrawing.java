...
public class PixelDrawing {
	...
	/** the area of the cell to draw, in DB units */
	...
	/** opaque layer of the window */
	private final int[] opaqueData;
	/** size of the opaque layer of the window */
	private final int total;
	/** the background color of the offscreen image */
	private int backgroundColor;
	/** the "unset" color of the offscreen image */
	private int backgroundValue;
	/** cache of port colors */
	private HashMap<PrimitivePort, EGraphics> portGraphicsCache = new HashMap<PrimitivePort, EGraphics>();

	// the transparent bitmaps
	/** the offscreen maps for transparent layers */
	private byte[][][] layerBitMaps;
	/** row pointers for transparent layers */
	private byte[][] compositeRows;
	/** the number of transparent layers */
	int numLayerBitMaps;
	/** the current transparent layers */
	private Color[] trueColorMap;
	/** the number of bytes per row in offscreen maps */
	private final int numBytesPerRow;
	/** the number of offscreen transparent maps made */
	private int numLayerBitMapsCreated;
	/** the technology of the window */
	...
	/** the map from layers to Patterned Opaque bitmaps */
	...
	public void printImage(double scale, Point2D offset, Cell cell, VarContext varContext, ElectricPrinter ep) {
		...
		if (ep.getDisplayAlgorithm() == 0) {
			...
			vd.render(this, scale, offset, cell, false, null, null, null, varContext, ep.getGreekSizeLimit(), ep
					.getGreekCellSizeLimit(), lv);
		}
		...
		if (numLayerBitMapsCreated > 0) {
			...
			for (int y = ly; y <= hy; y++) {
				...
				for (int x = lx; x <= hx; x++) {
					...
					// the value of Alpha starts at 0xFF, which means
					// "background"
					// opaque drawing typically sets it to 0, which means
					// "filled"
					// Text drawing can antialias by setting the edge values in
					// the range 0-254
			...
			// nothing in transparent layers: make sure background color is
			// right
			if (bounds == null) {
	...
	private void drawCell(Cell cell, Rectangle2D drawLimitBounds, boolean fullInstantiate, Orientation orient,
			AffineTransform prevTrans, Cell topCell) {
		renderedCells++;
	...
	private void drawNode(NodeInst ni, Orientation orient, AffineTransform trans, Cell topCell,
		...
		if (ni.isCellInstance()) {
			...
			if (canDrawText)
				showCellPorts(ni, trans, expanded, onPathDown);
