...
public class CIF1 implements CIF.CIFActions {
	 ...
	 *            the library to fill
	 * @param currentCells
	 *            this map will be filled with currentCells in Libraries found
	 *            in library file
	...
	private Rectangle getInterpreterBounds() {
		...
		for (FrontItem h : currentItemList) {
			...
			Point temp = new Point(obj.bb.l, obj.bb.b);

			Point comperror = transformPoint(temp);
			initMinMax(comperror);
			temp = temp.withX(obj.bb.r);
			comperror = transformPoint(temp);
			minMax(comperror);
			temp = temp.withY(obj.bb.t);
			comperror = transformPoint(temp);
			minMax(comperror);
			temp = temp.withX(obj.bb.l);
			comperror = transformPoint(temp);
			minMax(comperror);
			...
			temp = new Point(right, top);
			minMax(temp);
		}
		Rectangle ret = new Rectangle(getMinMaxMinX(), getMinMaxMinY(), getMinMaxMaxX() - getMinMaxMinX(),
				getMinMaxMaxY() - getMinMaxMinY());
		doneMinMax();
		popTransform();
		return ret;
	}
	...
	public void makeLabel(String name, Point pt) {
		...
		if (isInCellDefinition && cellScaleFactor != 1.0) {
			pt = pt.withX((int) Math.round(cellScaleFactor * pt.getX()));
			pt = pt.withY((int) Math.round(cellScaleFactor * pt.getY()));
		}
		obj.pos = pt;
		obj.name = name;
	...
	public void makeBox(int length, int width, Point center, int xr, int yr) {
		...
		if (isInCellDefinition && cellScaleFactor != 1.0) {
			...
			center = new Point((int) Math.round(cellScaleFactor * center.getX()),
					(int) Math.round(cellScaleFactor * center.getY()));
		}
		...
		int halfW = width / 2;
		int halfL = length / 2;
		if ((yr == 0 && (length % 2) == 0 && (width % 2) == 0 && (center.getX() - halfL) == tl
				&& (center.getX() + halfL) == tr && (center.getY() - halfW) == tb && (center.getY() + halfW) == tt)
				|| (xr == 0 && (length % 2) == 0 && (width % 2) == 0 && (center.getX() - halfW) == tl
						&& (center.getX() + halfW) == tr && (center.getY() - halfL) == tb && (center.getY() + halfL) == tt)) {
			// a Manhattan box
			...
			if (yr == 0) {
				...
				obj.bb.l = center.getX() - halfW;
				obj.bb.r = center.getX() + halfW;
				obj.bb.b = center.getY() - halfL;
				obj.bb.t = center.getY() + halfL;
			}
			if (isInCellDefinition) {
				// insert into symbol's guts
				obj.next = currentFrontSymbol.guts;
	...
	private Rectangle boundsBox(int length, int width, Point center, int xr, int yr) {
		...
		rotateMatrix(xr, yr);
		translateMatrix(center.getX(), center.getY());
	...
	public void makeWire(int width) {
		...
		FrontPath bbpath = new FrontPath(); // get a new path for bounding box
											// use
	...
	private Rectangle boundsWire(int width, FrontPath pPath) {
		...
		popTransform();
		Rectangle rect = new Rectangle(getMinMaxMinX() - half, getMinMaxMinY() - half, getMinMaxMaxX()
				- getMinMaxMinX() + half * 2, getMinMaxMaxY() - getMinMaxMinY() + half * 2);
		doneMinMax();
		return rect;
	}
	...
	private void findCallBounds(FrontCall object) {
		...
		findBounds(thisST); // get the bounding box of the symbol in its
							// FrontSymbol
		object.unID = thisST; // get this symbol's id

		pushTransform(); // set up a new frame of reference
		applyLocal(object.matrix);
		Point temp = new Point(thisST.bounds.l, thisST.bounds.b);
		Point comperror = transformPoint(temp);
		initMinMax(comperror);
		temp = temp.withX(thisST.bounds.r);
		comperror = transformPoint(temp);
		minMax(comperror);
		temp = temp.withY(thisST.bounds.t); // upper-right
		comperror = transformPoint(temp);
		minMax(comperror);
		temp = temp.withX(thisST.bounds.l);
		comperror = transformPoint(temp);
		minMax(comperror);
	...
	private void findBounds(FrontSymbol sym) {
		...
		while (ob != null) {
			// find bounding box for symbol calls, all primitive are done
			// already
			if (ob instanceof FrontCall) {
				findCallBounds((FrontCall) ob);
			}
			Point temp = new Point(ob.bb.l, ob.bb.b);
			if (first) {
				first = false;
				initMinMax(temp);
			} else {
			...
			temp = new Point(ob.bb.r, ob.bb.t);
			minMax(temp);
			ob = ob.next;
		}
	...
	/**
	 * Method to find a given symbol. If none, make a blank entry.
	 * 
	...
	private void scalePath(FrontPath src, FrontPath dest) {
		...
		for (int i = 0; i < limit; i++) {
			...
			temp = new Point((int) Math.round(cellScaleFactor * temp.getX()),
					(int) Math.round(cellScaleFactor * temp.getY()));
	...
	private void minMax(Point foo) {
		if (foo.getX() > minMaxStackRight[minMaxStackPtr]) {
			minMaxStackRight[minMaxStackPtr] = foo.getX();
		} else {
			if (foo.getX() < minMaxStackLeft[minMaxStackPtr]) {
				minMaxStackLeft[minMaxStackPtr] = foo.getX();
			}
		}
		if (foo.getY() > minMaxStackTop[minMaxStackPtr]) {
			minMaxStackTop[minMaxStackPtr] = foo.getY();
		} else {
			if (foo.getY() < minMaxStackBottom[minMaxStackPtr]) {
				minMaxStackBottom[minMaxStackPtr] = foo.getY();
			}
		}
	}
	...
	private Point transformPoint(Point foo) {
		...
		case TIDENT:
			return foo;
		case TTRANSLATE:
			return new Point((int) matrixStackTop.a31, (int) matrixStackTop.a32).add(foo);
		case TMIRROR:
			return foo.scale((matrixStackTop.a11 < 0) ? -1 : 1, (matrixStackTop.a22 < 0) ? -1 : 1);
		case TROTATE:
			return new Point((int) (foo.getX() * matrixStackTop.a11 + foo.getY() * matrixStackTop.a21),
					(int) (foo.getX() * matrixStackTop.a12 + foo.getY() * matrixStackTop.a22));
		default:
			return new Point((int) (matrixStackTop.a31 + foo.getX() * matrixStackTop.a11 + foo.getY()
					* matrixStackTop.a21),
					(int) (matrixStackTop.a32 + foo.getX() * matrixStackTop.a12 + foo.getY()
							* matrixStackTop.a22));
		}
	}
	...
	private void errorReport(String mess, int kind) {
		...
		// (resetInputBuffer ? 0 : 1)) + ": " + inputBuffer.toString());
		// }
		if (kind == FATALINTERNAL || kind == FATALINTERNAL || kind == FATALSEMANTIC || kind == FATALOUTPUT) {
			// numFatalErrors++;
		}
		switch (kind) {
		case FATALINTERNAL:
